### Exhibit 5 - Data Categories

### What This Exhibit Contains

This exhibit contains the source material for 'Data Categories'. It should be read as a standalone file document and cross-checked against the other exhibits. Key content includes: Status | Partial Names and emails | Employees and applicants | Basic personal data Passport/ID copies | Applicants | High concern

### How to Use This Exhibit

Read the exact wording rather than relying on the title.

Identify what is confirmed, what is uncertain and what is missing.

Use the exhibit number and document title when citing evidence in your answer.

Do not treat this exhibit alone as the full legal or commercial position.

Cross-check against the instructions, task brief and other exhibits before finalising your output.

### Full Exhibit Content

**Prepared by:** HR and onboarding team

**Status:** Partial

| Data category | Affected group | Sensitivity indicator |
| --- | --- | --- |
| Names and emails | Employees and applicants | Basic personal data |
| Passport/ID copies | Applicants | High concern |
| Payroll exports | Employees | Financial information |
| Medical absence notes | Employees | Sensitive context |
| Supplier contacts | Business contacts | Lower individual risk but still data |

### Expanded Exhibit Detail

The additional detail below expands the Full Exhibit Content only. It restates the matter-file evidence in a more developed form so the exhibit can be read consistently with the rest of the LawBridge pack.

## Source Status and Timing

- Prepared by: HR and onboarding team.

- Status: Partial.

## Detailed Matter Points

- Source detail: Prepared by is recorded as HR and onboarding team.

- Source detail: Status is recorded as Partial.

- For Data category 'Names and emails', the exhibit records Affected group: Employees and applicants; Sensitivity indicator: Basic personal data.

- For Data category 'Passport/ID copies', the exhibit records Affected group: Applicants; Sensitivity indicator: High concern.

- For Data category 'Payroll exports', the exhibit records Affected group: Employees; Sensitivity indicator: Financial information.

- For Data category 'Medical absence notes', the exhibit records Affected group: Employees; Sensitivity indicator: Sensitive context.

- For Data category 'Supplier contacts', the exhibit records Affected group: Business contacts; Sensitivity indicator: Lower individual risk but still data.

### Uncertainty and Follow-Up Points Within This Exhibit

- No express uncertainty wording was extracted from this exhibit, but the document should still be cross-checked against the other exhibits before it is used as proof of the full position.

## Additional Evidence-Use Detail

- This exhibit is short, so each individual wording choice matters: identify the person, system, date, document title, status label and any qualification before relying on it.

- Where the exhibit gives only a brief statement, the safer approach is to record the statement as a source fact and then request the supporting record, log, policy, email chain or approval trail.

### Student Extraction Boxes

### Box 1 - Key facts from this exhibit

> _Type your answer here._

## Box 2 - Risks, uncertainty or missing information

> _Type your answer here._

### Box 3 - How this exhibit affects the final output

> _Type your answer here._

## Box 4 - Follow-up questions to carry forward

> _Type your answer here._
