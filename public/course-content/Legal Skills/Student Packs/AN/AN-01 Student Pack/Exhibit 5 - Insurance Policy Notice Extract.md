### Exhibit 5 - Insurance Policy Notice Extract

### What This Exhibit Contains

This exhibit contains the source material for 'Insurance Policy Notice Extract'. It should be read as a standalone file document and cross-checked against the other exhibits. Key content includes: Status | Provided by client Relevant wording | Notification and consent The insured must notify the insurer as soon as reasonably practicable after becoming aware of an event that may give rise to a claim. The insured must not incur external forensic or public relations costs without prior written consent where reasonably possible.

### How to Use This Exhibit

Read the exact wording rather than relying on the title.

Identify what is confirmed, what is uncertain and what is missing.

Use the exhibit number and document title when citing evidence in your answer.

Do not treat this exhibit alone as the full legal or commercial position.

Cross-check against the instructions, task brief and other exhibits before finalising your output.

### Full Exhibit Content

**Document:** Cyber policy extract

**Status:** Provided by client

**Relevant wording:** Notification and consent

The insured must notify the insurer as soon as reasonably practicable after becoming aware of an event that may give rise to a claim. The insured must not incur external forensic or public relations costs without prior written consent where reasonably possible.

| Check item | Evidence in exhibit | Student note |
| --- | --- | --- |
| Has the broker been notified? |  |  |
| Have external costs already been incurred? |  |  |
| Does urgent containment justify immediate action? |  |  |

### Expanded Exhibit Detail

The additional detail below expands the Full Exhibit Content only. It restates the matter-file evidence in a more developed form so the exhibit can be read consistently with the rest of the LawBridge pack.

## Source Status and Timing

- Status: Provided by client.

- Document: Cyber policy extract.

## Detailed Matter Points

- Narrative point: The insured must notify the insurer as soon as reasonably practicable after becoming aware of an event that may give rise to a claim. The insured must not incur external forensic or public relations costs without prior written consent where reasonably...

- Source detail: Document is recorded as Cyber policy extract.

- Source detail: Status is recorded as Provided by client.

- Source detail: Relevant wording is recorded as Notification and consent.

- The exhibit records Check item: Has the broker been notified?.

- The exhibit records Check item: Have external costs already been incurred?.

- The exhibit records Check item: Does urgent containment justify immediate action?.

### Uncertainty and Follow-Up Points Within This Exhibit

- The wording 'may' in this exhibit uses possibility wording rather than confirmed fact; record it as a caveat or follow-up point, not as a final conclusion.

## Additional Evidence-Use Detail

- This exhibit is short, so each individual wording choice matters: identify the person, system, date, document title, status label and any qualification before relying on it.

- Where the exhibit gives only a brief statement, the safer approach is to record the statement as a source fact and then request the supporting record, log, policy, email chain or approval trail.

### Student Extraction Boxes

### Box 1 - Key facts from this exhibit

> _Type your answer here._

## Box 2 - Risks, uncertainty or missing information

> _Type your answer here._

### Box 3 - How this exhibit affects the final output

> _Type your answer here._

## Box 4 - Follow-up questions to carry forward

> _Type your answer here._
